angular.module('Directives', [])
  ...
  .directive('dlSearchMember', [function() {
    ...
    return {
      ...
      templateUrl: '/directives/searchMember.html',
      scope: false,
      controller: function($scope, $rootScope, $state, SendRequest, DataCache, ErrorDisplay, searchFactory) {
        ...
        $rootScope.getMemberAndVotes = function(name) {
          ...
          if (zipcode) {
            ...
            .then(function(response) {
              ...
              localStorage.setItem('memberData', JSON.stringify(data));
              // getAPIVotes off rootscope, on factory!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
              searchFactory.getAPIVotes(data.member.id);
              $state.go($state.current, {}, {reload: true});
            })
            .catch(function(err) {
              ErrorDisplay.errorMessage = 'No congresspeople matching the name '
